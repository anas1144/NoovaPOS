...
class SaleReturnRepository extends BaseRepository
{
    ...
    public function storeSaleReturn($input): SaleReturn
    {
        try {
            ...
            $saleReturnInputArray = Arr::only($input, [
                ...
                'paid_amount', 'payment_type', 'note', 'date', 'status', 'sale_id', 'posted_status',
            ]);

            // Set default posted_status if not provided (default to draft)
            if (!isset($saleReturnInputArray['posted_status'])) {
                $saleReturnInputArray['posted_status'] = SaleReturn::STATUS_DRAFT;
            }
            ...
            // Stock is added only if sale return is posted (draft returns don't affect stock)
            if (isset($saleReturnInputArray['posted_status']) && $saleReturnInputArray['posted_status'] == SaleReturn::STATUS_POSTED) {
            ...
            return $saleReturn->load('saleReturnItems.product');
        } catch (Exception $e) {
            DB::rollBack();
    ...
    public function postSaleReturn($id): SaleReturn
    {
        try {
            DB::beginTransaction();
            ...
            if ($saleReturn->posted_status == SaleReturn::STATUS_POSTED) {
                throw new UnprocessableEntityHttpException('This sale return has already been posted.');
            }

            // Apply all stock additions (sale returns add stock back to warehouse)
            foreach ($saleReturn->saleReturnItems as $saleReturnItem) {
                if (!$saleReturnItem->product_id || !$saleReturnItem->quantity) {
                    throw new UnprocessableEntityHttpException('Sale return item missing product_id or quantity.');
                }
                manageStock($saleReturn->warehouse_id, $saleReturnItem->product_id, $saleReturnItem->quantity);
                $this->stockMovementService()->logIn(
                    movementType: 'sale_return',
                    warehouseId: (int) $saleReturn->warehouse_id,
                    productId: (int) $saleReturnItem->product_id,
            ...
            // Mark as posted
            $saleReturn->update(['posted_status' => SaleReturn::STATUS_POSTED]);
