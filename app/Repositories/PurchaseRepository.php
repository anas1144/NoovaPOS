...
class PurchaseRepository extends BaseRepository
{
    ...
    public function postPurchase($id): Purchase
    {
        try {
            DB::beginTransaction();
            ...
            if ($purchase->posted_status == Purchase::STATUS_POSTED) {
                throw new UnprocessableEntityHttpException('This purchase has already been posted.');
            }

            // Apply all stock additions (purchases add stock to warehouse)
            foreach ($purchase->purchaseItems as $purchaseItem) {
                manageStock($purchase->warehouse_id, $purchaseItem->product_id, $purchaseItem->quantity);
            ...
            // Mark as posted
            $purchase->update(['posted_status' => Purchase::STATUS_POSTED]);
