...
class TransferRepository extends BaseRepository
{
    ...
    public function storeTransfer($input)
    {
        try {
            ...
            $TransferInputArray = Arr::only($input, [
                ...
                'note', 'date', 'status', 'posted_status',
            ]);
            
            // Always create as draft
            $TransferInputArray['posted_status'] = Transfer::STATUS_DRAFT;
    ...
    public function storeTransferItems($transfer, $input)
    {
        foreach ($input['transfer_items'] as $transferItem) {
            // Validate stock exists but don't apply changes yet (will happen when posted)
            ...
            if ($product) {
                ...
                // Stock will be transferred when transfer is posted, not when saved as draft
            } else {
    ...
    public function updateTransfer($input, $id)
    {
        try {
            ...
            foreach ($input['transfer_items'] as $key => $transferItem) {
                $transferItemNewIds[$key] = $transferItem['transfer_item_id'] ?? null;
                ...
                if (is_null($transferItem['transfer_item_id'])) {
                    ...
                    $item = $this->calculationTransferItems($transferItemArray);
                    TransferItem::whereId($transferItemArray['transfer_item_id'])->update(Arr::except($item, 'transfer_item_id'));
                }
            }
    ...
    /**
     * Post a transfer - Apply stock changes and mark as posted
     */
    public function postTransfer($id): Transfer
    {
        try {
            DB::beginTransaction();
            ...
            if ($transfer->posted_status == Transfer::STATUS_POSTED) {
                throw new UnprocessableEntityHttpException('This transfer has already been posted.');
            }

            // Apply all stock transfers
            foreach ($transfer->transferItems as $transferItem) {
                // Check if we have enough stock in source warehouse
                $fromStock = ManageStock::whereWarehouseId($transfer->from_warehouse_id)
                    ->whereProductId($transferItem->product_id)
                    ->first();

                if (!$fromStock || $fromStock->quantity < $transferItem->quantity) {
                    throw new UnprocessableEntityHttpException('Insufficient stock in source warehouse.');
                }
                ...
                $this->stockMovementService()->logOut(
                    ...
                    productId: (int) $transferItem->product_id,
            ...
            // Mark as posted
            $transfer->update(['posted_status' => Transfer::STATUS_POSTED]);
