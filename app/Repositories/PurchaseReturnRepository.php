...
class PurchaseReturnRepository extends BaseRepository
{
    ...
    public function postPurchaseReturn($id): PurchaseReturn
    {
        try {
            DB::beginTransaction();
            ...
            if ($purchaseReturn->posted_status == PurchaseReturn::STATUS_POSTED) {
                throw new UnprocessableEntityHttpException('This purchase return has already been posted.');
            }

            // Apply all stock deductions (purchase returns remove stock from warehouse)
            foreach ($purchaseReturn->purchaseReturnItems as $purchaseReturnItem) {
                if (!$purchaseReturnItem->product_id || !$purchaseReturnItem->quantity) {
                    throw new UnprocessableEntityHttpException('Purchase return item missing product_id or quantity.');
                }
                manageStock($purchaseReturn->warehouse_id, $purchaseReturnItem->product_id, -$purchaseReturnItem->quantity);
                $this->stockMovementService()->logOut(
                    movementType: 'purchase_return',
                    warehouseId: (int) $purchaseReturn->warehouse_id,
                    productId: (int) $purchaseReturnItem->product_id,
                    quantity: (float) $purchaseReturnItem->quantity,
                    referenceType: PurchaseReturn::class,
                    referenceId: (int) $purchaseReturn->id,
                    tenantId: $purchaseReturn->tenant_id
                );
            ...
            // Mark as posted
            $purchaseReturn->update(['posted_status' => PurchaseReturn::STATUS_POSTED]);
