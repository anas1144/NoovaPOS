...
import { fetchSales, postSale } from "../../store/action/salesAction";
...
const Sales = (props) => {
    ...
    const handleConfirmPost = () => {
        if (itemToPost) {
            ...
            postSale(itemToPost.id, navigate, () => {
                setPostingId(null);
            ...
            cell: (row) =>
                row.reference_code === "Total" ? null : (
                    <ActionDropDownButton
                        item={row}
                        goToEditProduct={getPermission(allConfigData?.permissions, Permissions.EDIT_SALE) && row.posted_status === 0 ? goToEdit : null}
                        isPdfIcon={true}
                        ...
                        isDeleteMode={getPermission(allConfigData?.permissions, Permissions.DELETE_SALE) && row.posted_status === 0}
