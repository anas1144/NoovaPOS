...
export const postAdjustment = (adjustmentId, navigate, setIsPosting) => async (dispatch) => {
    try {
        await apiConfig
            .post(apiBaseURL.ADJUSTMENTS + "/" + adjustmentId + "/post")
            .then((response) => {
                dispatch({
                    type: adjustMentActionType.EDIT_ADJUSTMENTS,
                    payload: response.data.data,
                ...
                dispatch(
                    addToast({
                        text: "Adjustment posted successfully!",
