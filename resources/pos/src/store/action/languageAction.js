...
export const toggleLanguageStatus = (languageId) => async (dispatch) => {
    apiConfig
        .post(apiBaseURL.LANGUAGES + "/" + languageId + "/toggle-status")
        .then((response) => {
            // Determine the appropriate message key based on the response
            const isEnabled = response.data.message.includes('enabled');
            const messageKey = isEnabled
                ? 'language.enabled.success.message'
                : 'language.disabled.success.message';

            dispatch(
                addToast({
                    text: getFormattedMessage(messageKey),
                })
            );
            dispatch(fetchLanguages({order_By:"created_at", direction: "desc" }));
        })
