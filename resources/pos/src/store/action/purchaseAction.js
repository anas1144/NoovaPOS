...
export const postPurchase = (purchaseId, navigate, callback) => async (dispatch) => {
    try {
        await apiConfig
            .post(apiBaseURL.PURCHASES + "/" + purchaseId + "/post")
            .then((response) => {
                dispatch({
                    type: purchaseActionType.EDIT_PURCHASE,
                    payload: response.data.data,
                ...
                dispatch(
                    addToast({
                        text: getFormattedMessage("purchase.success.post.message"),
