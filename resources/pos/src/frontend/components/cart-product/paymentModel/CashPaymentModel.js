...
const CashPaymentModel = (props) => {
    ...
    const handleSave = (event, shouldPrint = false) => {
        // compute total paid from payment details or single received_amount
        const totalPaid = cashPaymentValue.payment_details && cashPaymentValue.payment_details.length > 0
            ? cashPaymentValue.payment_details.reduce((sum, payment) => {
                const amt = parseFloat(payment.amount);
                return sum + (isNaN(amt) ? 0 : amt);
            }, 0)
            : parseFloat(cashPaymentValue.received_amount) || 0;

        // Validate payment detail entries if status is PAID or PARTIAL
        if (
        ...
        // Block submit/print if total paid is less than grand total
        if (Number(totalPaid) < Number(grandTotal)) {
            const outstanding = (Number(grandTotal) - Number(totalPaid)).toFixed(2);
            dispatch(
                addToast({
                    text: intl.formatMessage({
                        id: 'pos.payment.outstanding.error',
                        defaultMessage: 'Outstanding balance: {amount}',
                    }, { amount: currencySymbolHandling(allConfigData, settings.attributes && settings.attributes.currency_symbol, outstanding) }),
                    type: toastType.ERROR,
                })
            );
            return;
