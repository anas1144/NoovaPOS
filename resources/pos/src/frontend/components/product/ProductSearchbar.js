...
const ProductSearchbar = (props) => {
    ...
    const filterProduct = (() => {
        ...
        // Filter out products that are out of stock AND don't allow out-of-stock sales
        return posAllProducts.filter((item) => 
            item.attributes.stock.quantity > 0 || item.attributes?.allow_out_of_stock_sale === true
        );
    })()
    .sort((a, b) => {
        // Sort by product code numerically
        const codeA = parseInt(a.attributes.code) || 0;
        const codeB = parseInt(b.attributes.code) || 0;
        return codeA - codeB;
    })
    .map((item) => ({
    ...
    const onProductSearch = (code) => {
        ...
        const foundProduct = posAllProducts.find(
            (product) =>
        ...
        const allowOutOfStockSale = foundProduct?.attributes?.allow_out_of_stock_sale === true;
        ...
        if (finalSearch[0]) {
            ...
            if (
                ...
                if (!allowOutOfStockSale && filterQty >= singleProduct[0]) {
                    dispatch(
                        addToast({
                    ...
                    setUpdateProducts((updateProducts) =>
                        updateProducts.map((item) =>
                            (item.code === code ||
                                ...
                                item.code === code.code) &&
                            // || item.name === code.name
                            (allowOutOfStockSale || singleProduct[0] > item.quantity)
    ...
    const handleOnSelect = (result) => {
        const selectedProduct = posAllProducts.find(
            (item) => item.id === result.id || 
                      item.attributes.code === result.code || 
                      item.attributes.name === result.name
        );
        
        const allowOutOfStockSale = selectedProduct?.attributes?.allow_out_of_stock_sale === true;
    
        if (!allowOutOfStockSale) {
            if (selectedProduct?.attributes?.stock?.quantity <= 0) {
                dispatch(
                    addToast({
                        text: getFormattedMessage(
    ...
    const handleOnSearch = (string) => {
        if (string.trim() !== "") {
            ...
            const filteredProducts = showOutOfStockProducts
                ? posAllProducts
                : posAllProducts.filter(item => 
                    item.attributes.stock.quantity > 0 || item.attributes?.allow_out_of_stock_sale === true
                );
            ...
            if (codeSearch.length > 0) {
               const isOutOfStock = codeSearch[0].attributes?.stock?.quantity <= 0;
               const allowOutOfStockSale = codeSearch[0].attributes?.allow_out_of_stock_sale === true;
